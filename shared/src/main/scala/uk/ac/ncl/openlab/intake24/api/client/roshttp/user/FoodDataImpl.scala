...
class FoodDataImpl(val requestHandler: RequestHandler) extends FoodDataService with HttpServiceUtils {
  def lookup(locale: String, description: String, existing: Seq[String], limit: Option[Int], algorithm: Option[String]): Future[Either[ApiError, LookupResult]] = {
    ...
    authRequestWithResponse[LookupResult](Method.GET, s"/foods/$locale/lookup", accessToken, queryParameters)
  }
}
